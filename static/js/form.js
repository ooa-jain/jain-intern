...
// ── APPS SCRIPT URL ───────────────────────────────────────────────────────────
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwEcizvV1E8VKhjFXRsjwfp1IfG4D9wchODgBIcurK_6t1T002oIw6T9eqa3WL81gTg/exec";

// ── CHAPTERS ──────────────────────────────────────────────────────────────────
const CHAPTERS = [
  {
    color:'#FFE066', sfx:'KAPOW!', avatar:'🦸',
    title:'Hero Profile', sub:'Your origin story starts here',
    fields:[
      {type:'two-col',fields:[
        {type:'text',key:'name',label:'Your Name',hint:'e.g. Alex Chen'},
        {type:'text',key:'sid', label:'Student ID', hint:'e.g. S12345678'}
      ]},
      {type:'two-col',fields:[
        {type:'text',key:'prog',label:'Programme',hint:'e.g. Computer Science'},
        {type:'text',key:'dur', label:'Duration',  hint:'e.g. Jan – May 2025'}
      ]},
  ...
  {
    color:'#C8F7C5', sfx:'ZAP!', avatar:'🧠',
    title:'Learn to Learn', sub:'Level up! What new powers did you gain?',
    fields:[
  ...
  {
    color:'#FFCFDF', sfx:'POW!', avatar:'💡',
    title:'Learn to Think', sub:'How did you crack the case?',
    fields:[
  ...
  {
    color:'#D0E8FF', sfx:'WHAM!', avatar:'🤝',
    title:'Learn to Collaborate', sub:'Every hero needs a sidekick',
    fields:[
      {type:'two-col',fields:[
        {type:'text',key:'team',     label:'The Squad',hint:'e.g. 3 engineers, 1 PM'},
        {type:'text',key:'team_role',label:'My Role',  hint:'e.g. Frontend Dev'}
      ]},
      {type:'textarea',  key:'collab_win', label:'Squad Win',      hint:'e.g. Organised a whiteboard session…',rows:3},
      {type:'textarea',  key:'collab_fail',label:'Squad Challenge', hint:'e.g. Differing opinions on design…',rows:3},
  ...
  {
    color:'#FFF0CC', sfx:'BOOM!', avatar:'⚖️',
    title:'Learn to Regulate', sub:'Heroes manage their emotions too',
    fields:[
  ...
  {
    color:'#E8D5FF', sfx:'BLAM!', avatar:'🛠️',
    title:'The Arsenal', sub:'Every hero has gadgets',
    fields:[
      ...
      {type:'textarea',  key:'best_tool',label:'Tool I Am Most Proud Of Using',hint:'What can you do now that you couldn\'t before?',rows:3}
    ]
  },
  {
    color:'#C5F7EB', sfx:'ZING!', avatar:'📅',
    title:'Journey Timeline', sub:'3 legendary moments',
    fields:[
      {type:'text',    key:'m1t',label:'Mission 1 — Start',hint:'e.g. The Great Code Review of Week 2'},
  ...
  {
    color:'#FFE4CC', sfx:'CRASH!', avatar:'💥',
    title:'The Impact', sub:'How did you move the needle?',
    fields:[
  ...
  {
    color:'#FFD6D6', sfx:'WHOOSH!', avatar:'🌟',
    title:'Mentor Spotlight', sub:'Shoutout to your Yoda',
    fields:[
      {type:'two-col',fields:[
        {type:'text',key:'mentor_name',label:'Mentor Name',hint:'e.g. Dr Jane Wong'},
        {type:'text',key:'mentor_role',label:'Their Role', hint:'e.g. Senior Engineer'}
      ]},
  ...
  {
    color:'#D5F0FF', sfx:'NEXT!', avatar:'🚀',
    title:"What's Next", sub:'The sequel is already written',
    fields:[
...
function renderField(f){
  if(f.type==='two-col')
    return `<div class="two-col">${f.fields.map(renderField).join('')}</div>`;

  if(f.type==='text')
    return `<div class="field-block">
      <div class="field-label">${f.label}</div>
      <input type="text" data-key="${f.key}" value="${(D[f.key]||'').replace(/"/g,'&quot;')}" placeholder="${f.hint||''}">
    </div>`;
  ...
  if(f.type==='textarea')
    return `<div class="field-block">
      ...
      <textarea data-key="${f.key}" rows="${f.rows||3}">${D[f.key]||''}</textarea>
    </div>`;
  ...
  if(f.type==='checkboxes'){
    ...
    return `<div class="field-block">
      ...
      <div class="cb-group">
        ${f.options.map(o=>`
          <span class="cb-pill${sel.includes(o)?' on':''}" data-grp="${f.key}" data-val="${o}">
            <span class="cb-check">
              ${sel.includes(o)?`<svg width="9" height="9" viewBox="0 0 9 9"><path d="M1.5 4.5l2 2L7.5 2" stroke="#fffef7" stroke-width="1.8" stroke-linecap="round" fill="none"/></svg>`:''}
            </span>${o}
          </span>`).join('')}
      </div>
    </div>`;
  ...
  if(f.type==='rating'){
    ...
    return `<div class="field-block">
      <div class="field-label">${f.label}</div>
      <div class="star-row">
        ${[1,2,3,4,5].map(i=>`<span class="star${i<=v?' lit':''}" data-rk="${f.key}" data-rv="${i}">★</span>`).join('')}
      </div>
    </div>`;
  }
  return '';
...
function dots(){
  return CHAPTERS.map((_,i)=>
    `<div class="cdot${i<cur?' done':i===cur?' active':''}">${i<cur?'✓':i+1}</div>`
  ).join('');
}
  ...
  document.getElementById('app').innerHTML=`
    ...
    <div class="panel-body">
      ...
      <div class="nav-row">
        ...
        <button class="nav-btn ${isLast?'final':'primary'}" id="btn-next">
          ${isLast?'MISSION COMPLETE! →':'NEXT CHAPTER →'}
        </button>
      </div>
    </div>`;
...
function bind(){
  document.querySelectorAll('input[type=text]').forEach(el=>
    el.addEventListener('input',e=>sv(e.target.dataset.key,e.target.value)));
  document.querySelectorAll('textarea').forEach(el=>
    el.addEventListener('input',e=>sv(e.target.dataset.key,e.target.value)));
  document.querySelectorAll('[data-grp]').forEach(el=>
    el.addEventListener('click',()=>{ tc(el.dataset.grp,el.dataset.val); render(); }));
  document.querySelectorAll('[data-rk]').forEach(el=>
    el.addEventListener('click',()=>{ R[el.dataset.rk]=parseInt(el.dataset.rv); render(); }));
  document.getElementById('btn-back').addEventListener('click',()=>{ cur--; render(); });
  document.getElementById('btn-next').addEventListener('click',()=>{
    if(cur<CHAPTERS.length-1){ cur++; render(); }
    else submitAndShowSummary();
  });
}

// ── SUBMIT TO APPS SCRIPT ────────────────────────────────────────────────────
async function submitAndShowSummary(){
  const payload={
  ...
  try {
    // URL-encoded body + no-cors: bypasses CORS, Apps Script reads e.parameter
    ...
    // no-cors = opaque response, cannot read it, but POST reached Apps Script
    showOverlay(false);
...
function showOverlay(show){
  ...
  if(show){
    if(!el){
      ...
      el.innerHTML=`<div class="saving-box"><span>⚡</span>SAVING TO SHEETS…</div>`;
...
const SUMMARIES=[
  ...
  ()=>sr('Setback & Comeback',v('setback'))+sr('Time Management',v('time_mgmt'))+sr('Wellbeing',cc('wellbeing'))+sr('Stress Level',cc('stress')),
  ...
  ()=>sr('Delivered',v('contribution'))+sr('Value Created',v('value'))+sr('Knowledge to Share',v('school_back')),
  ()=>sr('Mentor',v('mentor_name'))+sr('Their Role',v('mentor_role'))+sr('Lesson Learned',v('mentor_lesson'))+sr('Power Level',st('mentor_rating')),
  ()=>sr('Top 3 Power-ups',v('takeaways'))+sr('Next Arc Goals',v('goals'))+sr('Message to Next Hero',v('advice'))
];
...
function renderSummary(){
  ...
  document.getElementById('app').innerHTML=`
    ...
    <div class="summary-header">
      ...
      <div style="font-size:14px;color:#1a1a2e;margin-top:4px;font-style:italic">All 10 chapters documented. Your hero journey is saved!</div>
    </div>
    <div class="summary-panel">
      ${chapters}
      <button class="dl-btn" id="dl-btn">⬇ DOWNLOAD MY SUMMARY (.TXT)</button>
...
function download(){
  const lines=[
    ...
    `Lesson:\n${v('mentor_lesson')}`,`Rating: ${st('mentor_rating')}`,'',
    "WHAT'S NEXT",'-'.repeat(40),
    `Power-ups:\n${v('takeaways')}`,`Goals:\n${v('goals')}`,`Advice:\n${v('advice')}`,'',
